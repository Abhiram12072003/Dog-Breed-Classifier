...
modelcnn = load_model('cnn.h5')
modelrnn = load_model('rnn.h5')
modelmlp = load_model('mlp.h5')
...
def pipeline_model_rnn(path):
    img = image.load_img(path,target_size=(299,299))
    img = image.img_to_array(img)
    img = img/255.0
    img = np.expand_dims(img,axis=0)

    pred = modelrnn.predict(img)
    max_preds = []
    pred = pred[0]
    for i in range(5):
        name = labels[pred.argmax()]
...
def pipeline_model_mlp(path):
    img = image.load_img(path,target_size=(299,299))
    img = image.img_to_array(img)
    img = img/255.0
    img = np.expand_dims(img,axis=0)

    pred = modelmlp.predict(img)
    max_preds = []
    pred = pred[0]
    for i in range(5):
        name = labels[pred.argmax()]
